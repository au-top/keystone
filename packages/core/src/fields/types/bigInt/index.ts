        ...
        async validateInput(args) {
          ...
          if (
            ...
            args.addValidationError(
              `${fieldLabel} ${i18nLang.Fields.Components.Bigint.Index.IsRequired}`
            );
          }
          if (typeof value === 'number') {
            if (validation?.min !== undefined && value < validation.min) {
              args.addValidationError(
                `${fieldLabel} ${i18nLang.Fields.Components.Bigint.Index.MustBeGreaterThanOrEqualTo} ${validation.min}`
              );
            }

            if (validation?.max !== undefined && value > validation.max) {
              args.addValidationError(
                `${fieldLabel} ${i18nLang.Fields.Components.Bigint.Index.MustBeLessThanOrEqualTo} ${validation.max}`
              );
            }
          }
